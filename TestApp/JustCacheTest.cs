...
public class JustCacheTest
{
    public static List<(string title, string setElapsed, string getElapsed)> Start()
    {
        ...
        results.Add(("JustCache with 10 items", test1.set, test1.get));
        var test2 = TestCachePerformance(100);
        results.Add(("JustCache with 100 items", test2.set, test2.get));
        var test3 = TestCachePerformance(1000);
        results.Add(("JustCache with 1000 items", test3.set, test3.get));
        var test4 = TestCachePerformance(100000);
        results.Add(("JustCache with 100000 items", test4.set, test4.get));
        var test5 = TestCachePerformance(1000000);
        results.Add(("JustCache with 1000000 items", test5.set, test5.get));
        ...
        results.Add(("JustCache Cleanup", cleanupElapsed.ToString(), cleanupElapsed.ToString()));
    ...
    static (string set, string get) TestCachePerformance(int itemCount)
    {
        ...
        stopwatch.Restart();
        var cachedJson = JustCache.GetString(key);
        stopwatch.Stop();
